...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        if ($deposit->order_id) {
            ...
            $transaction->details = 'Added With Balance As Payment For Ordering Foods Via ' . $deposit->gatewayCurrency()->name;
            ...
            $transaction->charge = $deposit->charge;
            $transaction->trx_type = '-';
            $transaction->details = 'Subtracted For Ordering Foods';
            $transaction->trx = $deposit->trx;
            $transaction->save();

            //Saving Final Balance
            $user->save();
            ...
            foreach ($order->details as $item) {
                $foodList .= '# '.$item->food->name.' x '.$item->qty.' = '.$gnl->cur_text.' '.showAmount($item->price).'<br>';
            }

            notify($user, 'USER_ORDER_CONFIRMATION', [
                'order_code' => $order->order_code,
                'd_time' => $order->restaurant->d_time,
                'food_list' => $foodList,
                'currency' => $gnl->cur_text,
            ...
            notify($order->restaurant, 'RESTAURANT_ORDER_CONFIRMATION', [
                'order_code' => $order->order_code,
                'd_time' => $order->restaurant->d_time,
                'food_list' => $foodList,
                'currency' => $gnl->cur_text,
    ...
    public function reject(Request $request)
    {
        ...
        if ($deposit->order_id) {
            ...
            notify($deposit->user, 'USER_ORDER_CANCELED', [
                'order_code' => $order->order_code,
                'd_charge' => showAmount($order->d_charge),
                'food_list' => $foodList,
                'currency' => $general->cur_text,
